### 警告! 請不要再跑這個湯底來自 ngc3351 的腳本了, 有些東西不能混用! ###
### 可以以 radex-pipeline_modiPath-add6d.py 代替。基本上效果是一樣的, 但細節還是要成套比較安全###
...
def radex_flux(i,j,k,m,n):
    ...
    flux_0 = np.genfromtxt(dataPath+'/output_'+model+'_'+molecule_0+'/'+Tk+'_'+prej_r+'e'+n_h2+'_'+codex+'e'+N_co+'.out', skip_header=13)[:,11]
    flux_1 = np.genfromtxt(dataPath+'/output_'+model+'_'+molecule_1+'/'+Tk+'_'+prej_r+'e'+n_h2+'_'+codex+'e'+N_co+'_'+x13co+'.out', skip_header=13)[:,11]
