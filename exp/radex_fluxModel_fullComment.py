...
### ----------------------------- Save Models into .npy Files ------------------------------- ###
'''
the loop for filee saving was modi by qing (20260317)
為了改檔名的時候能他媽舒服一點
(still radex_pipeline.py in Eltha's words)

至於這下面的東西, 我認為應該和進行大號 radex_io 的部分分在兩個不同腳本
但因為相依的細細變數 (pre-processing 那邊造成的) 太他媽多了
所以目前還是先放在一起 瘋了都
'''
# --------------------------- def radex_flux(): **from bayes repo** ---------------------------- #
...
'''
這邊就是主要修改的地方
如果這邊沒有補上關於容器之間關係的詳細的解釋, 那麼請見茜草本子
那邊應該有紙本版本的說明
'''
...
'''#mole_info
molecule spieces and the initial shape of each one's flux:
co: 3d, 13co: 4d, c18o: 5d
idky now @@
'''
flux_model = {}
'''
put flux model here, "flux" and "flux_6d".
structure belike:
flux_model[f'{molespiece}-{transis}'] = {
        "flux": ,
        "flux_6d": ,
    }
'''
...
for molename in flux_model.keys(): # Save "flux" into .npy
    np.save(f'{npyPath}/flux_nd_{molename}.npy', flux_model[molename]["flux"]) # (filename) modi by qing (20260317)
