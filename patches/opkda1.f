...
C Call DPREP to do matrix preprocessing operations. --------------------
     ...
     1   RWORK(LACOR), IA, JA, RWORK(LWM), RWORK(LWM), 
C This line is modi by qing on 20251018 (fix error?)
     2   IPFLAG,
     3   F,
     4   JAC)
...
C
      ...
      DO 50 II = 1,N
        ...
        DO 40 J = JMIN,JMAX
C This ifelse is rebuild by qing on 20251018 (fix warning)
          IF (JA(J) < II) THEN
            GOTO 10
          ELSE IF (JA(J) == II) THEN
            GOTO 40
          ELSE
            GOTO 30
          END IF
...
c----for each nonzero entry a(vi,vj)
      do 6 vi=1,n
        ...
        do 5 j=jmin,jmax
          vj = ja(j)
c This ifelse is rebuild by qing on 20251-18 (fix warning)
c ori: if (vj-vi) 2, 5, 4
          if (vj<vi) then
            goto 2
          else if (vj==vi) then
            goto 5
...
c----for each vertex vi in ek
      ...
      do 10 ilp=1,ilpmax
        ...
        vi = v(i)
c This ifelse is rebuild by qing on 20251-18 (fix warning)
c ori: if (last(vi))  1, 10, 8
        if (last(vi)<0) then
          goto 1
        else if (last(vi)==0) then
          goto 10
        else if (last(vi)>0) then
          goto 8
        end if
...
c  ******  if not, see if kth column can overlap the previous one  *****
  11    if (jlmin .gt. jlptr)  go to 15
        ...
        do 12 j=jlmin,jlptr
c This ifelse is rebuild by qing on 20251-18 (fix warning)
c ori: if (jl(j) - qm)  12, 13, 15
          if (jl(i)<qm) then
            goto 12
          else if (jl(i)==qm) then
            goto 13
          else if (jl(i)>qm) then
            goto 15
          end if
...
c  ******  if not, see if kth row can overlap the previous one  ********
  28    if (jumin .gt. juptr)  go to 32
        ...
        do 29 j=jumin,juptr
c This ifelse is rebuild by qing on 20251-18 (fix warning)
c ori: if (ju(j) - qm)  29, 30, 32
          if (ju(j)<qm) then
            goto 29
          else if (ju(j)==qm) then
            goto 30
          else if (ju(j)>qm) then
            goto 32
          end if
...
C Call DPREPI to do matrix preprocessing operations. -------------------
     ...
     1   RWORK(LACOR), IA, JA, IC, JC, RWORK(LWM), RWORK(LWM), 
     2   IPFLAG,
     3   RES, 
     4   JAC, 
     5   ADDA)
