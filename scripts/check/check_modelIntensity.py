...
update: 2026-09-19: Fix dimension problem and generalize this script.
        2026-09-20: Add residual map of predicted flux and observed data.
'''
...
if mode == 'regular':
    ...
    bsize = 3.2
    regridPath = f'{projectRoot}/data/regrid_map_nyq'
    ...
    moles_info = [('co-10',   3.0, 1100), ('13co-10', 3.0, 70),
                  ('co-21',   3.0, 1200), ('13co-21', 3.0, 180),
                  ('c18o-21', 3.0, 35),   ('co-32',   3.0, 1000)]
elif mode == 'resolved':
    modelPath = f'{projectRoot}/data/model_no-hco+' # model grid 沒有 re不resolve 的問題
    phyArray = np.load(f'{modelPath}/phy_plain-model_Tk-nH2-Nco.npy')
    bsize = 0.41
    regridPath = f'{projectRoot}/data/regrid_map_resolved'
...
for molename, nsig, _ in moles_info:
    fluxx[molename] = {
        ...
        "observed": np.load(f'{regridPath}/mom0_{molename}_smooth{bsize}as_{nsig}sigma_regrid.npy'),
        "error": np.load(f'{regridPath}/emap_{molename}_smooth{bsize}as_regrid.npy'),
        "predicted": np.full(datashape, np.nan)
    }
...
for molename in fluxx.keys():
    ...
    fluxx[molename]["predicted"][validMask] = mflux[validIdx_phy, validIdx_phib]
    '''
...
# Normalized residual and residual
fig, ax = plt.subplots(2, 3, figsize=(14, 7))
ax_flat = ax.flatten() # 壓成 1d 這樣可以用洄圈
for i, m in enumerate(moles_info):
    residual = fluxx[m[0]]["predicted"] - fluxx[m[0]]["observed"]
    residual_nor = residual / fluxx[m[0]]["error"]
    splot = ax_flat[i].imshow(residual_nor, cmap='coolwarm', origin='lower') # 怎麼弄到已0為中心啊屁ㄧ
    ax_flat[i].set_title(f'{m[0]} residudal map')
    cbar = fig.colorbar(splot, ax=ax_flat[i], fraction=0.046, pad=0.04) # 精心調整的小數值
plt.tight_layout() # 神奇妙妙工具
